export function waitHelpersCase() {
  return `
    ...
    await assertRejects(
      () => page.waitForSelector("#does-not-exist", { timeout: 200 }),
      "timed out",
      "waitForSelector throws TimeoutError for missing elements"
    );
    await assertRejects(
      () => page.waitForSelector("#never-visible", { timeout: 400, state: "visible" }),
      "timed out",
      "waitForSelector state:visible times out for hidden elements"
    );
  `;
}
...
export function fetchHelpersCase() {
  return `
    ...
    const serverRedirect = await fetch.server(baseUrl + "/api/redirect", { timeout: 5_000 });
    assertEqual(serverRedirect, "server text fixture", "serverFetch follows redirects");

    await assertRejectsAny(
      () => fetch.server(baseUrl + "/api/slow?ms=1200&case=server-timeout-" + Date.now(), { timeout: 50 }),
      "serverFetch honors timeout"
    );

    await assertRejects(
      () => fetch.server(baseUrl + "/api/error", { timeout: 5_000 }),
      "HTTP 500",
      "serverFetch reports HTTP errors"
    );

    const browserText = await fetch.browser("/api/text", { timeout: 5_000 });
    ...
    const browserRedirect = await fetch.browser("/api/redirect", { timeout: 5_000 });
    assertEqual(browserRedirect, "server text fixture", "browserFetch follows redirects");

    const browserAbsolute = await fetch.browser(baseUrl + "/api/text", { timeout: 5_000 });
    assertEqual(browserAbsolute, "server text fixture", "browserFetch accepts absolute URLs");

    await assertRejectsAny(
      () => fetch.browser("/api/slow?ms=1200&case=browser-timeout-" + Date.now(), { timeout: 50 }),
      "browserFetch honors timeout"
    );

    await assertRejects(
      () => fetch.browser("/api/error", { timeout: 5_000 }),
      "HTTP 500",
...
export function cdpJsHelpCase() {
  return `
    ...
    const mouseHelp = help("page.mouse");
    assertIncludes(mouseHelp, "page.mouse.move(", "help lists mouse methods by public path");
    assertIncludes(mouseHelp, "page.mouse.down(", "help lists low-level mouse controls");
    assertIncludes(
      help("page.mouse.down"),
      "Press a mouse button",
      "help resolves an exact mouse path"
    );
    assertIncludes(
      help("page.keyboard.down"),
      "Dispatch a keydown event",
      "help disambiguates the keyboard path"
    );
    const ambiguousHelp = help("down");
    assertIncludes(ambiguousHelp, "Ambiguous helper name", "help reports ambiguous bare names");
    assertIncludes(ambiguousHelp, "page.keyboard.down", "ambiguous help lists the keyboard path");
    assertIncludes(ambiguousHelp, "page.mouse.down", "ambiguous help lists the mouse path");
