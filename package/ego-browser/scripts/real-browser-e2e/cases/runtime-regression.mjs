export function runtimeRegressionCase() {
  return `
    ...
    /* Resolver failure classes on waitForSelector: a permanent (ambiguous) selector
       ...
       TimeoutError. Official only covers the happy path. */
    ...
    await assertRejects(
      () => page.waitForSelector("#no-such-element-xyz", { timeout: 1000 }),
      "timed out",
      "waitForSelector throws for a transient missing element"
    );
    assert(
    ...
    assertEqual(
      (await fetch.server(baseUrl + "/api/bytes?n=0", { timeout: 5_000 })).length,
      0,
      "serverFetch returns empty body for zero-byte request"
    );
    assertEqual(
      (await fetch.server(baseUrl + "/api/bytes?n=1", { timeout: 5_000 })).length,
      1,
      "serverFetch returns one byte for single-byte request"
    );
    const bytes = await fetch.server(baseUrl + "/api/bytes?n=4096", { timeout: 5_000 });
    ...
    await assertRejects(
      () => fetch.server(baseUrl + "/api/status?code=404", { timeout: 5_000 }),
      "HTTP 404",
      "serverFetch reports 4xx HTTP errors"
    );
    await assertRejects(
      () => fetch.browser("/api/status?code=404", { timeout: 5_000 }),
      "HTTP 404",
