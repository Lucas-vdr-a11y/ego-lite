...
export const dragDropScenarioCase = scenarioCase(
  ...
  `
      ...
      await observedGesture(page, "board before invalid pointer drop", async (pointer) => {
        await pointer.move(handleBox.x + handleBox.width / 2, handleBox.y + handleBox.height / 2);
        await pointer.down();
        await pointer.move(10, 10, { steps: 12 });
        await pointer.up();
      });
      ...
      await observedGesture(page, "board before review pointer drop", async (pointer) => {
        await pointer.move(handleBox.x + handleBox.width / 2, handleBox.y + handleBox.height / 2);
        await pointer.down();
        await pointer.move(targetBox.x + targetBox.width / 2, targetBox.y + targetBox.height * 0.72, { steps: 12 });
        await pointer.up();
      });
      ...
      await observedDragTo(page, htmlSource, htmlTarget);
      ...
      await observedAction(page, page.getByRole("button", { name: "Reset board" }), "click");
      ...
      await observedGesture(page, "board before reverse pointer drop", async (pointer) => {
        await pointer.move(reverseHandleBox.x + reverseHandleBox.width / 2, reverseHandleBox.y + reverseHandleBox.height / 2);
        await pointer.down();
        await pointer.move(reverseHandleBox.x + reverseHandleBox.width / 2 + 12, reverseHandleBox.y + reverseHandleBox.height / 2 + 12, { steps: 4 });
        await pointer.move(progressTargetBox.x + progressTargetBox.width / 2, progressTargetBox.y + Math.min(progressTargetBox.height * 0.5, 100), { steps: 20 });
      ...
      await observedAction(page, page.getByRole("button", { name: "Reset board" }), "click");
      await observedAction(page, page.getByRole("button", { name: "Move checkout reassurance to review" }), "click");
